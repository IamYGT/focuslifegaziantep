...
?>
										<tr id="item-<?php echo $row["ekip_ust_id"]; ?>" role="row" class="even"> 
											...
											<td class=""><?php if(isset($row["ekip_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/teams/<?php echo $row["ekip_resim"]; ?>" alt="<?php echo $row["ekip_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["ekip_baslik"]; ?>"><?php } ?></td>
					...
					if(!empty($_POST['ekip_baslik']) || !empty($_POST['ekip_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET ekip_baslik = ?, ekip_seo = ?, ekip_aciklama = ?, ekip_description = ?, ekip_resim = ?, dil_id = ?, ekip_durum = ?, ekip_ust_id = ?");
								$insert->execute(array($ekip_baslik[$key], Seo_Link_Cevir($ekip_baslik[$key]), $ekip_aciklama[$key], $ekip_description[$key], $ekip_resim, $key, $ekip_durum, $LastID));
...
<div class="col-xl-8 col-xxl-8">
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Ekip Ekle</h4>
                            </div>
                            <div class="card-body"> 
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="ekip_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="">
															</div>
														</div>   
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Ünvan</label>
																<input type="text" name="ekip_aciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Ünvan" required="">
															</div>
														</div>
													<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i>Detay Adresi</label>
																<input type="text" name="ekip_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Detay Adresi" required="">
															</div>
														</div>
													</div>
					...
					if(isset($submitControl)){
						if(!empty($_POST['ekip_baslik']) || !empty($_POST['ekip_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  ekip_baslik = ?, ekip_seo = ?, ekip_aciklama = ?, ekip_description = ?, ekip_resim = ?, ekip_durum = ? WHERE ekip_ust_id = ? AND dil_id = ?");
									$update->execute(array($ekip_baslik[$key], Seo_Link_Cevir($ekip_baslik[$key]), $ekip_aciklama[$key],  $ekip_description[$key], $ekip_resim, $ekip_durum, $id, $key));
								}
							}
							if ($update) {
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Başlık</label>
																<input type="text" name="ekip_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="" value="<?php echo GetTableValue("ekip_baslik",TABLE,"where ekip_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Ünvan</label>
																<input type="text" name="ekip_aciklama[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Ünvan" required="" value="<?php echo GetTableValue("ekip_aciklama",TABLE,"where ekip_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
													<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Detay Adresi</label>
																<input type="text" name="ekip_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Detay Adresi" required="" value="<?php echo GetTableValue("ekip_description",TABLE,"where ekip_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
