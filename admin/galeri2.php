							...
							<div id="example5_wrapper" class="dataTables_wrapper no-footer">
								<table class="table display mb-4 dataTablesCard card-table dataTable no-footer" id="example5" role="grid" aria-describedby="example5_info">
									<thead>
										...
										<th class="sorting" tabindex="0" aria-controls="example5" rowspan="1" colspan="1" aria-label="Date Applied: activate to sort column ascending" style="width: 115;">Başlık</th>
...
<?php
									$list = $db->query("SELECT * FROM ".TABLE." WHERE dil_id = '".LANGUAGE_DEFAULT."' ORDER BY row ASC LIMIT $baslangic,$limit"); 
										if ($list->rowCount()){
											foreach($list as $row){ 
?>
										<tr id="item-<?php echo $row["galeri2_ust_id"]; ?>" role="row" class="even"> 
											<td class="sortable ui-sortable-handle" style="width:20px"><i class="fa fa-sort"></i></td>
											<td class=""><?php echo $row["galeri2_baslik"]; ?></td>
											<td class=""><?php if(isset($row["galeri2_resim"])){ ?><img class="mr-3 " width="60" src="../uploads/gallery/<?php echo $row["galeri2_resim"]; ?>" alt="<?php echo $row["galeri2_baslik"]; ?>"><?php }else{ ?><img class="mr-3 " width="60" src="images/no_images.png" alt="<?php echo $row["galeri2_baslik"]; ?>"><?php } ?></td>
											<td class=""><?php echo $row["galeri2_tarih"]; ?></td>
											<td class="sorting_1">
												<div class="d-flex">
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=files2&id=<?php echo $row["galeri2_ust_id"]; ?>"><i class="fas fa-images" aria-hidden="true"></i></a>
													<a class="contact-icon mr-3" href="<?php echo AREA; ?>?do=edit&id=<?php echo $row["galeri2_ust_id"]; ?>"><i class="fas fa-edit" aria-hidden="true"></i></a>
													<a class="contact-icon" href="<?php echo AREA; ?>?do=delete&id=<?php echo $row["galeri2_ust_id"]; ?>"><i class="fas fa-trash-alt"></i></a>
												</div>
											</td>
										</tr> 
<?php 	
...
<?php 
				...
				if(isset($submitControl)){
					if(!empty($_POST['galeri2_baslik']) || !empty($_POST['galeri2_aciklama'])) {
						$LastID = $db->query("SELECT galeri2_id FROM ".TABLE." ORDER BY galeri2_id DESC LIMIT 1")->fetch(PDO::FETCH_ASSOC);
						$LastID = $LastID["galeri2_id"]+1;
						$upload = new Upload($_FILES['galeri2_resim']);
						if ($upload->uploaded) { 
								$upload->file_auto_rename = true;
								$upload->process("../uploads/gallery");
							if ($upload->processed){  
								$galeri2_resim = $upload->file_dst_name;
							} else { 
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET galeri2_baslik = ?, galeri2_seo = ?, galeri2_aciklama = ?, galeri2_resim = ?, dil_id = ?, galeri2_durum = ?, galeri2_ust_id = ?");
								$insert->execute(array($galeri2_baslik[$key], Seo_Link_Cevir($galeri2_baslik[$key]), $galeri2_aciklama[$key], $galeri2_resim, $key, $galeri2_durum, $LastID));
													...
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																...
																<input type="text" name="galeri2_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="">
															</div>
														</div>  
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Description</label>
																<input type="text" name="galeri2_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Description" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Keywords</label>
																<input type="text" name="galeri2_keywords[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Keywords" required="">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="galeri2_aciklama[<?php echo $LANGUAGE_CODE; ?>]"></textarea>
															</div>
														</div>
													</div>
                        ...
                        <div class="card">
                            <div class="card-body p-3"> 
									...
									<div class="fileinput fileinput-new" data-provides="fileinput">
										...
										<div>
											<span class="btn btn-primary btn-sm btn-file">
												...
												<input type="file" name="galeri2_resim" accept="image/*" id="galeri2_resim">
											</span>
								...
								<div class="form-group"> 
									...
									<select name="galeri2_durum" class="form-control">
...
<?php 
				...
				$row_check = $db->prepare("SELECT * FROM ".TABLE." WHERE galeri2_ust_id = ?");
				$row_check->execute(array($id)); 
				if ($row_check->rowCount() > 0) {
					if(isset($submitControl)){
						if(!empty($_POST['galeri2_baslik']) || !empty($_POST['galeri2_aciklama'])) {
							$image_info = $db->query("SELECT * FROM ".TABLE." WHERE galeri2_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
								$upload = new Upload($_FILES['galeri2_resim']);  
								if(!isset($_FILES['galeri2_resim']['name'])) {
									$galeri2_resim = null;
								} else if(!empty($_FILES['galeri2_resim']['name'])) {
									$upload->file_auto_rename = true; 
									$upload->process("../uploads/gallery");
									if ($upload->processed) {
										$galeri2_resim = $upload->file_dst_name;
									...
									$galeri2_resim = $image_info["galeri2_resim"];
								} 
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  galeri2_baslik = ?, galeri2_seo = ?, galeri2_aciklama = ?, galeri2_resim = ?, galeri2_durum = ? WHERE galeri2_ust_id = ? AND dil_id = ?");
									$update->execute(array($galeri2_baslik[$key], Seo_Link_Cevir($galeri2_baslik[$key]), $galeri2_aciklama[$key], $galeri2_resim, $galeri2_durum, $id, $key));
								}
							}
							if ($update) {
					...
					$row_info = $db->query("SELECT * FROM ".TABLE." WHERE galeri2_ust_id = {$id}")->fetch(PDO::FETCH_ASSOC);
?>
												...
												<div class="step-tab-panel" data-step="step<?php echo $LANGUAGE_ID; ?>">
													<div class="row">
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Başlık</label>
																<input type="text" name="galeri2_baslik[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Başlık" required="" value="<?php echo GetTableValue("galeri2_baslik",TABLE,"where galeri2_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Description</label>
																<input type="text" name="galeri2_description[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Description" required="" value="<?php echo GetTableValue("galeri2_description",TABLE,"where galeri2_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Keywords</label>
																<input type="text" name="galeri2_keywords[<?php echo $LANGUAGE_CODE; ?>]" class="form-control" placeholder="Keywords" required="" value="<?php echo GetTableValue("galeri2_keywords",TABLE,"where galeri2_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?>">
															</div>
														</div>
														<div class="col-lg-12 mb-2">
															<div class="form-group">
																<label class="text-label"><i class="flag-icon flag-icon-<?php echo $LANGUAGE_CODE; ?> icon-2x"></i> Açıklama</label>
																<textarea class="form-control ckeditor" name="galeri2_aciklama[<?php echo $LANGUAGE_CODE; ?>]"><?php echo GetTableValue("galeri2_aciklama",TABLE,"where galeri2_ust_id = {$id} and dil_id = '{$LANGUAGE_CODE}' "); ?></textarea>
															</div>
														</div>
													</div>
													<input type="hidden" name="form_<?php echo $LANGUAGE_CODE; ?>" value="<?php echo $LANGUAGE_CODE; ?>" />
					...
					<div class="col-xl-4 col-xxl-4">
                        <div class="card">
                            <div class="card-body p-3">  
								...
								<label>Resim</label>
								<small class="form-text text-muted">Seçmiş olduğunuz resim veri içeriğinde kullanılmaktadır.</small> 
								<?php if(empty($row_info["galeri2_resim"])) {?>
										<div class="fileinput fileinput-new" data-provides="fileinput">
											<div class="fileinput-preview thumbnail" data-trigger="fileinput" style="width: 100%;height: 190px;line-height: 190px;">
								...
								<?php }else{?> 
										<div class="fileinput fileinput-exists" data-provides="fileinput">
											...
											<div class="fileinput-preview fileinput-exists thumbnail" style="max-width: 303px; max-height: 190px; line-height: 190px;"><img src="../uploads/gallery/<?php echo $row_info["galeri2_resim"]; ?>" class="img-fluid img-thumbnail"></div>
											<div>
												<span class="btn btn-primary btn-sm btn-file">
													<span class="fileinput-new"><span class="fui-image"></span>Resim Seç</span>
													<span class="fileinput-exists"><span class="fui-gear"></span>Değiştir</span>
													<input type="file" name="galeri2_resim" id="galeri2_resim" >
								...
								<div class="form-group"> 
									<label for="">Yayın Durumu</label>
									<select name="galeri2_durum" class="form-control">
										<option value="1" <?php if ($row_info["galeri2_durum"] == "1") { echo "selected";}?>>Aktif</option>
										<option value="2" <?php if ($row_info["galeri2_durum"] == "2") { echo "selected";}?>>Pasif</option> 
									</select>
								</div> 
                            </div>
                        </div>
...
?>
				...
				<div class="col-xl-12"> 
					<div class="card">
						<div class="card-header">
							<h4 class="card-title">Yüklenmiş galeri2</h4>
						</div>
						<div class="card-body"> 						
							<div class="row">
								<?php
								$list = $db->query("SELECT * FROM files2 WHERE ustid = {$id} AND itable = 1");
								foreach($list as $row){
								?>
								<div class="col-lg-3">
									<div class="card">
									   <a href="javascript:;"><img src="../uploads/files2/<?php echo $row["name"]; ?>" class="img-fluid" style="width:300px;height:150px"></a>
...
<?php 
				...
				if($type == "gallery"){
					$row_check = $db->prepare("SELECT * FROM files2 WHERE id = ?");
					$row_check->execute(array($id)); 
					if ($row_check->rowCount() > 0) {
						$update = $db->exec("DELETE FROM files2 WHERE id = {$id};");
						if($update) {
							echo '<meta http-equiv="refresh" content="1;url='.AREA.'?do=files2&id='.$id2.'">';
					...
					$row_check->execute(array($id)); 
					if ($row_check->rowCount() > 0) {
						$update = $db->exec("DELETE FROM ".TABLE." WHERE galeri2_ust_id = {$id};");
						if($update) {
...
<?php if($do == 'files2'){ ?>
  ...
  <script src="js/dropzone/dropzone.js"></script>
	...
	var myDropzone = new Dropzone("div#myDrop", {
		 paramName: "files2", // The name that will be used to transfer the file
		 addRemoveLinks: true,
		 uploadMultiple: true,
	 ...
	 myDropzone.on("success", function(file, message){
		$("#msg").html(message);
		setTimeout(function(){
			window.location.href='galeri2?<?php echo $_SERVER["REDIRECT_QUERY_STRING"]; ?>'},0
		);
	 });
