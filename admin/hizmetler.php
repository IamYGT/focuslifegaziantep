					...
					if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET haber_baslik = ?, haber_seo = ?, haber_aciklama = ?,   haber_kisaaciklama = ?,  haber_resim = ?, dil_id = ?, haber_durum = ?, haber_ust_id = ?");
								$insert->execute(array($haber_baslik[$key], Seo_Link_Cevir($haber_baslik[$key]), $haber_aciklama[$key], $haber_kisaaciklama[$key], $haber_resim, $key, $haber_durum, $LastID));
						...
						if(!empty($_POST['haber_baslik']) || !empty($_POST['haber_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									...
									$update = $db->prepare("UPDATE ".TABLE." SET  haber_baslik = ?, haber_seo = ?, haber_aciklama = ?,   haber_kisaaciklama = ?,  haber_resim = ?, haber_durum = ? WHERE haber_ust_id = ? AND dil_id = ?");
									$update->execute(array($haber_baslik[$key], Seo_Link_Cevir($haber_baslik[$key]), $haber_aciklama[$key],    $haber_kisaaciklama[$key],  $haber_resim, $haber_durum, $id, $key));
								}
							}
							if ($update) {
