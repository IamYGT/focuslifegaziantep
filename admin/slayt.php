					...
					if(!empty($_POST['slayt_baslik']) || !empty($_POST['slayt_aciklama'])) {
						...
						foreach ($_POST AS $k=>$v) {
							...
							if (substr($k,0,5) == "form_") {
								...
								$insert = $db->prepare("INSERT INTO ".TABLE." SET slayt_baslik = ?,  slayt_resim = ?, dil_id = ?, slayt_durum = ?, slayt_ust_id = ?");
								$insert->execute(array($slayt_baslik[$key],  $slayt_resim, $key, $slayt_durum, $LastID));
					...
					if(isset($submitControl)){
						if(!empty($_POST['slayt_baslik']) || !empty($_POST['slayt_aciklama'])) {
							...
							foreach ($_POST AS $k=>$v) {
								...
								if (substr($k,0,5) == "form_") {
									$key = str_replace("form_","",$k);
									$update = $db->prepare("UPDATE ".TABLE." SET  slayt_baslik = ?,  slayt_resim = ?, slayt_durum = ? WHERE slayt_ust_id = ? AND dil_id = ?");
									$update->execute(array($slayt_baslik[$key], $slayt_resim, $slayt_durum, $id, $key));
								}
							}
							if ($update) {
